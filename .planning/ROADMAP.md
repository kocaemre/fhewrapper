...
**Goal**: A connected wallet can decrypt its confidential balance for any ERC-7984 token via a correct EIP-712 user-decryption flow — built early because it is the only way to *see* the result of wrap and unwrap, so it exists to verify them.
**Mode:** mvp
**Depends on**: Phase 1
...
**Success Criteria** (what must be TRUE):
  ...
  2. Decryption also works for an ERC-7984 token *outside* the registry via paste-an-address (and/or auto-detect), proving the "any token" claim.
